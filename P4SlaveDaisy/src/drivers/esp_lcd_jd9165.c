 ...
 *
 * JD9165BA MIPI-DSI panel driver implementation.
 * Adapted from Espressif reference code for Guition JC1060P470C.
 */
...
// ---------------------------------------------------------------------------
// Vendor-specific init sequence for JD9165BA (Guition JC1060P470C 1024×600)
// ---------------------------------------------------------------------------
...
static esp_err_t panel_jd9165_invert_color(esp_lcd_panel_t *panel, bool invert);
static esp_err_t panel_jd9165_mirror(esp_lcd_panel_t *panel, bool mx, bool my);
...
esp_err_t esp_lcd_new_panel_jd9165(const esp_lcd_panel_io_handle_t io,
    ...
    jd9165_vendor_config_t *vendor_config =
        (jd9165_vendor_config_t *)panel_dev_config->vendor_config;
    ESP_RETURN_ON_FALSE(vendor_config &&
...
static esp_err_t panel_jd9165_del(esp_lcd_panel_t *panel) {
    ...
    if (jd9165->reset_gpio_num >= 0) {
        gpio_reset_pin((gpio_num_t)jd9165->reset_gpio_num);
    }
    jd9165->del(panel);
...
// ---------------------------------------------------------------------------
// Init — send vendor DCS commands then DPI init
// ---------------------------------------------------------------------------
static esp_err_t panel_jd9165_init(esp_lcd_panel_t *panel) {
    jd9165_panel_t *jd9165 = (jd9165_panel_t *)panel->user_data;
...
static esp_err_t panel_jd9165_reset(esp_lcd_panel_t *panel) {
    ...
    if (jd9165->reset_gpio_num >= 0) {
        gpio_set_level((gpio_num_t)jd9165->reset_gpio_num, !jd9165->flags.reset_level);
...
static esp_err_t panel_jd9165_mirror(esp_lcd_panel_t *panel, bool mx, bool my) {
    jd9165_panel_t *jd9165 = (jd9165_panel_t *)panel->user_data;
    esp_lcd_panel_io_handle_t io = jd9165->io;
    uint8_t val = jd9165->madctl_val;
    ESP_RETURN_ON_FALSE(io, ESP_ERR_INVALID_STATE, TAG, "invalid panel IO");
    if (mx) val |= JD9165_CMD_GS_BIT; else val &= ~JD9165_CMD_GS_BIT;
    if (my) val |= JD9165_CMD_SS_BIT; else val &= ~JD9165_CMD_SS_BIT;
    ESP_RETURN_ON_ERROR(esp_lcd_panel_io_tx_param(io, LCD_CMD_MADCTL, (uint8_t[]){val}, 1),
                        TAG, "send command failed");
    jd9165->madctl_val = val;
    return ESP_OK;
}
