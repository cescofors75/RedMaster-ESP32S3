...
constexpr uint8_t kShedVoiceLimit  = 24u;

/* Exact output saturator used by the Rust/WASM core. It keeps the summed
 * Monte-Carlo cloud musical without handing multi-volt peaks to the master. */
static inline float RaySoftClip(float x)
{
                             ...
                             size_t blockFrames)
{
    ...
    /* Reproduce the Web curve literally: its default Character=35 means
     * t=0.35, 231 grains/s, 122 ms grains and ~28 concurrent rays. The old
     * embedded normalization turned 35 into t=1 and demanded ~80 voices. */
    ...
    const float evolutionCurve = blockEvolution_ * blockEvolution_;
    /* Evolution should smear the scene, not turn it into short repeated
     * syllables. Longer overlapping rays make the recursive field breathe. */
    blockGrainSeconds_ = fminf(
        0.24f, rawGrainSeconds * (1.0f + 0.55f * evolutionCurve));
    blockRecursiveGain_ = Clamp01(
        evolutionCurve * (0.20f + recursiveEnvelope_ * 0.45f));
    ...
    /* Preserve the source as the body of the drone. Only a minority of rays
     * enter the harmonic field, even at maximum evolution. */
    blockHarmonicBlend_ = 0.32f * evolutionCurve;
    blockShimmerChance_ = 0.07f * evolutionCurve;
    /* No discontinuity above zero: low evolution only drifts the focus;
     * recursive rays appear gradually and never add full-level copies. */
    blockBounceChance_ = 0.18f * evolutionCurve;
    blockBounceDepth_ = blockEvolution_ < 0.45f
                            ? 0u
                            : (blockEvolution_ < 0.82f ? 1u : 2u);
    blockFocusDepth_ = blockEvolution_ < 0.12f
                           ? 0u
                           : (blockEvolution_ < 0.70f
                                  ? 1u
                                  : (blockEvolution_ < 0.92f ? 2u : 3u));
    blockFocusSpread_ = 0.22f + 0.10f * blockEvolution_;
    blockFocusDrift_ = 0.28f + 0.22f * blockEvolution_;
    blockFocusBirthRate_ = 0.15f + 0.55f * evolutionCurve;
    ...
    const float concurrency
        = fmaxf(1.0f,
                blockBaseDensity_ * blockGrainSeconds_ * materialDuration);
    ...
    const uint8_t blockVoiceLimit = qualityShed_
                                        ? kShedVoiceLimit
                                        : (materialTarget_ == 3u
                                               ? kGlassVoiceLimit
                                               : kMaxVoices);
...
float RaydroneDsp::NextHarmonicRatio()
{
    ...
    /* Weighted ambient voicing. The old round-robin POWER sequence made
     * transients neigh/bark as a conspicuous repeating arpeggio. */
    const float u = Random01();
    float ratio;
    if(u < 0.14f)      ratio = 0.5f;   /* sub octave */
    else if(u < 0.26f) ratio = 0.75f;  /* lower fourth */
    else if(u < 0.68f) ratio = 1.0f;   /* body/root */
    else if(u < 0.90f) ratio = 1.5f;   /* fifth */
    else               ratio = 2.0f;   /* shimmer octave */
    if(ratio <= 1.0f && Random01() < blockShimmerChance_)
        ratio *= 2.0f;
                             ...
                             uint8_t depth)
{
    ...
    /* Per-ray duration jitter breaks correlated grain endings and removes the
     * hoofbeat/pack rhythm from recursive scenes. */
    const float durationJitter = 0.82f + Random01() * 0.36f;
    const float durationSamples = fmaxf(
        1.0f, grainSeconds * sampleRate_ * materialDuration * durationJitter);
                              ...
                              uint8_t voiceLimit)
{
    ...
    const float timeScatter = 0.12f + blockEvolution_ * 0.28f;
    ageSeconds += (Random01() - 0.5f) * 2.0f * timeScatter;
    ageSeconds = Clamp(ageSeconds,
                       blockFocusMinSeconds_,
                       blockFocusMaxSeconds_);
    const float position = static_cast<float>(tapeWrite_)
                           - ageSeconds * sampleRate_;
    const float childDuration = grainSeconds * (0.72f + Random01() * 0.72f);
    const float childGain = gain * (0.38f + Random01() * 0.10f);
    PlaceVoice(position,
               childDuration,
               childGain,
               width,
                          ...
                          float& outputR)
{
    ...
    const uint8_t voiceLimit = qualityShed_
                                   ? kShedVoiceLimit
                                   : (materialTarget_ == 3u
                                          ? kGlassVoiceLimit
                                          : kMaxVoices);
    ...
    while(activeIndex < activeCount_)
    {
        ...
        if(phase >= 1.0f)
        {
            ...
            const float parentGain = voice.gain;
        ...
        /* Web uses Catmull-Rom. Spend those four SDRAM reads only on rays
         * whose pitch is far enough from unity to benefit audibly; ordinary
         * drone rays stay on the cheaper linear path. Shed always uses linear. */
        const bool pitchedRay = voice.step < 0.98f || voice.step > 1.02f;
        const float raw = (!qualityShed_ && pitchedRay)
                              ? ReadTapeCatmull(voice.position)
                              : ReadTapeLinear(voice.position);
    ...
    /* The Web renderer always soft-clips its wet output. The historical Daisy
     * port accidentally hard-clamped at ±4 and then distorted in the master. */
    chorusedL = RaySoftClip(chorusedL * wetVolume);
    chorusedR = RaySoftClip(chorusedR * wetVolume);
